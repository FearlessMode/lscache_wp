...
class LiteSpeed_Cache_ThirdParty_WpForo
{
	...
	public static function wpforo_hook_when_shutdown(){
		...
		if (wpforo_is_admin() && isset($_GET['page']) && $_GET['page'] == 'wpforo-forums') {
			if((isset($_POST['wpforo_submit']) && isset($_REQUEST['forum']) && isset($_GET['action']))
				...
				LiteSpeed_Cache_Tags::add_purge_tag(self::CACHETAG_COMMON);
			}
		}
	...
	public static function purge_tag_topic($topicid){
		LiteSpeed_Cache_Tags::add_purge_tag(self::CACHETAG_TOPIC . $topicid);
		LiteSpeed_Cache_Tags::add_purge_tag(self::CACHETAG_FORUM);
		LiteSpeed_Cache_Tags::add_purge_tag(self::CACHETAG_FRONTPAGE);
	}
	...
	public static function purge_tag_topic_add($args){
		// LiteSpeed_Cache_Tags::add_purge_tag(self::CACHETAG_FORUM . $args['forumid']);
		LiteSpeed_Cache_Tags::add_purge_tag(self::CACHETAG_FORUM);
		LiteSpeed_Cache_Tags::add_purge_tag(self::CACHETAG_FRONTPAGE);
	}
	...
	public static function cache_tags(){
		...
		if(!empty($wpforo->current_object['template'])){
			if($wpforo->current_object['template'] == 'forum'){
				LiteSpeed_Cache_Tags::add_cache_tag(self::CACHETAG_FRONTPAGE);
				LiteSpeed_Cache_Tags::add_cache_tag(self::CACHETAG_COMMON);
			}
			if($wpforo->current_object['template'] == 'topic'){
				LiteSpeed_Cache_Tags::add_cache_tag(self::CACHETAG_FORUM . $wpforo->current_object['forum']['forumid']);
				LiteSpeed_Cache_Tags::add_cache_tag(self::CACHETAG_FORUM);
				LiteSpeed_Cache_Tags::add_cache_tag(self::CACHETAG_COMMON);
			}
			if($wpforo->current_object['template'] == 'post'){
				LiteSpeed_Cache_Tags::add_cache_tag(self::CACHETAG_TOPIC . $wpforo->current_object['topic']['topicid']);
				LiteSpeed_Cache_Tags::add_cache_tag(self::CACHETAG_COMMON);
			}
		}
	}
